...
fn handle_add(pool: &PgPool, args: &Value) -> Result<(String, bool)> {
    ...
    handle.block_on(async {
        ...
        Ok::<_, anyhow::Error>((serde_json::to_string_pretty(&output)?, false))
    })
}
...
fn handle_get_counts(pool: &PgPool, args: &Value) -> Result<(String, bool)> {
    let thread_id = args["thread_id"]
        .as_i64()
        .ok_or_else(|| anyhow::anyhow!("Missing required argument: 'thread_id'"))?;
...
async fn main() -> Result<()> {
    ...
    let add_handler: ToolHandler = Box::new(move |args: &Value| handle_add(&p_add, args));
    ...
    let counts_handler: ToolHandler = Box::new(move |args: &Value| handle_get_counts(&p_cnt, args));
